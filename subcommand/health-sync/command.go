...
func (c *Command) realRun() error {
	...
	// We reuse the same token that the mesh-init would have
	// written to the shared volume.
	var tokenMeta *config.TokenMeta
	if c.config.ConsulLogin.Enabled {
		tokenMeta = &config.TokenMeta{
			TokenFile: c.config.ConstructConsulTokenFilePath(),
		}
	}
	serverConnMgrCfg, err := c.config.ConsulServerConnMgrConfig(taskMeta, tokenMeta)
	if err != nil {
